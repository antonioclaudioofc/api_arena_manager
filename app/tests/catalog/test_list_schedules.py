...
def test_list_catalog_schedules_by_court_without_auth(db, client):
...
def test_list_catalog_schedules_with_auth(db, client):
    ...
    response = client.get(
        f"/catalog/courts/{court.id}/schedules",
...
def test_list_catalog_schedules_empty(db, client):
    owner = user_factory(db)
    arena = arena_factory(db, owner.id)
    court = court_factory(db, arena.id)
...
def test_list_catalog_schedules_nonexistent_court(db, client):
    response = client.get("/catalog/courts/999/schedules")

    assert response.status_code == 404


def test_list_catalog_schedules_returns_correct_fields(db, client):
    owner = user_factory(db)
    arena = arena_factory(db, owner.id)
    court = court_factory(db, arena.id)

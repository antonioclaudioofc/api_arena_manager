...
def test_list_catalog_arenas_without_auth(db, client):
...
def test_list_catalog_arenas_with_auth(db, client):
...
def test_list_catalog_arenas_empty(db, client):
    response = client.get("/catalog/arenas")
...
def test_get_specific_catalog_arena(db, client):
    owner = user_factory(db)
    arena = arena_factory(
        db, owner.id, name="Arena Especial", city="São Paulo")

    response = client.get(f"/catalog/arenas/{arena.id}")
...
def test_get_nonexistent_catalog_arena(db, client):
    response = client.get("/catalog/arenas/999")
